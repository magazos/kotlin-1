...
fun test(x : Int?, a : A?) {
    x<!NI;UNSAFE_CALL, OI;UNSAFE_CALL!>.<!>plus(1)
    x?.plus(1)
    x <!NI;UNSAFE_OPERATOR_CALL, OI;UNSAFE_OPERATOR_CALL!>+<!> 1
    <!UNSAFE_CALL!>-<!>x
    x<!UNSAFE_CALL!>.<!>unaryMinus()
    x?.unaryMinus()

    a<!NI;UNSAFE_CALL, OI;UNSAFE_CALL!>.<!>plus(1)
    a?.plus(1)
    a <!UNSAFE_INFIX_CALL!>plus<!> 1
    a <!NI;UNSAFE_OPERATOR_CALL, OI;UNSAFE_OPERATOR_CALL!>+<!> 1
    <!UNSAFE_CALL!>-<!>a
